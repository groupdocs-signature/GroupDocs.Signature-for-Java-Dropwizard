...
public class SignatureServiceImpl implements SignatureService {
    ...
    private void createDirectories() {
        new File(getFullDataPath(signatureConfiguration.getDataDirectory(), CERTIFICATE_DATA_DIRECTORY.getPath())).mkdirs();
        new File(getFullDataPath(signatureConfiguration.getDataDirectory(), IMAGE_DATA_DIRECTORY.getPath())).mkdirs();
        new File(getFullDataPath(signatureConfiguration.getDataDirectory(), IMAGE_UPLOADED_DATA_DIRECTORY.getPath())).mkdirs();

        new File(getFullDataPath(signatureConfiguration.getDataDirectory(), STAMP_DATA_DIRECTORY.getXMLPath())).mkdirs();
        new File(getFullDataPath(signatureConfiguration.getDataDirectory(), STAMP_DATA_DIRECTORY.getPreviewPath())).mkdirs();

        new File(getFullDataPath(signatureConfiguration.getDataDirectory(), QRCODE_DATA_DIRECTORY.getXMLPath())).mkdirs();
        new File(getFullDataPath(signatureConfiguration.getDataDirectory(), QRCODE_DATA_DIRECTORY.getPreviewPath())).mkdirs();

        new File(getFullDataPath(signatureConfiguration.getDataDirectory(), BARCODE_DATA_DIRECTORY.getXMLPath())).mkdirs();
        new File(getFullDataPath(signatureConfiguration.getDataDirectory(), BARCODE_DATA_DIRECTORY.getPreviewPath())).mkdirs();

        new File(getFullDataPath(signatureConfiguration.getDataDirectory(), TEXT_DATA_DIRECTORY.getXMLPath())).mkdirs();
        new File(getFullDataPath(signatureConfiguration.getDataDirectory(), TEXT_DATA_DIRECTORY.getPreviewPath())).mkdirs();
    }
    ...
    public OpticalXmlEntity saveOpticalCode(SaveOpticalCodeRequest saveOpticalCodeRequest) {
        ...
        if (QR_CODE.equals(saveOpticalCodeRequest.getSignatureType())) {
            // get preview path
            previewPath = getFullDataPath(signatureConfiguration.getDataDirectory(), QRCODE_DATA_DIRECTORY.getPreviewPath());
            // get xml file path
            xmlPath = getFullDataPath(signatureConfiguration.getDataDirectory(), QRCODE_DATA_DIRECTORY.getXMLPath());
            // generate unique file names for preview image and xml file
            collection.add(new QrCodeSigner(signatureData, signatureDataEntity).signImage());
        } else {
            // get preview path
            previewPath = getFullDataPath(signatureConfiguration.getDataDirectory(), BARCODE_DATA_DIRECTORY.getPreviewPath());
            // get xml file path
            xmlPath = getFullDataPath(signatureConfiguration.getDataDirectory(), BARCODE_DATA_DIRECTORY.getXMLPath());
            // generate unique file names for preview image and xml file
            collection.add(new BarCodeSigner(signatureData, signatureDataEntity).signImage());
        }
        String encodedImage = createAndSaveOpticalCode(signatureData, signatureDataEntity, previewPath, xmlPath, collection);
     ...
     */
    private String createAndSaveOpticalCode(OpticalXmlEntity signatureData, SignatureDataEntity signatureDataEntity, String previewPath, String xmlPath, SignatureOptionsCollection collection) {
        try {
